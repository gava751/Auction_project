...
public interface LotRepository extends JpaRepository<Lot, Long> {
  ...
  @Query(
      "SELECT l FROM Lot l WHERE l.status = 'ACTIVE' AND l.endTime > CURRENT_TIMESTAMP "
  ...
  List<com.auction.platform.domain.Lot> findByStatusAndEndTimeBefore(
      com.auction.platform.domain.LotStatus status, java.time.LocalDateTime dateTime);

  @Query(
      "SELECT l FROM Lot l WHERE l.status = 'COMPLETED' "
