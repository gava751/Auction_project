...
public class BidService {
  ...
  public void placeManualBid(String userEmail, BidRequest request) {
    ...
    Lot lot =
        lotRepository
            .findByIdWithPessimisticLock(request.lotId())
  ...
  public void setupAutoBid(String userEmail, Long lotId, BigDecimal maxLimit) {
    ...
    AutoBid autoBid =
        autoBidRepository.findByLotIdAndUserId(lotId, user.getId()).orElse(new AutoBid());
    autoBid.setLot(lot);
  ...
  private void processAutoBids(Lot lot, Long currentLeaderId) {
    List<AutoBid> autoBids =
        autoBidRepository.findByLotIdAndIsActiveTrueOrderByMaxLimitDesc(lot.getId());
    ...
    if (newPrice.compareTo(lot.getCurrentPrice()) > 0
        && highest.getMaxLimit().compareTo(lot.getCurrentPrice()) > 0) {
      saveBidAndUpdateLot(lot, highest.getUser(), newPrice);
      log.info(
          "Авто-биддер пользователя {} перебил цену до {}", highest.getUser().getId(), newPrice);
    }
  ...
  private void validateLotIsActive(Lot lot) {
    if (!lot.getStatus().name().equals("ACTIVE")
        || lot.getEndTime().isBefore(LocalDateTime.now())) {
