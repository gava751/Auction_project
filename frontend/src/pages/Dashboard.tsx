...
import {Ban, CheckCircle, FileCheck, FolderTree, Gavel, Plus, ShieldPlus, Trash2, Users} from 'lucide-react';
    ...
    if (user?.role !== 'ROLE_ADMIN') {
        return (
            <div className="p-8 max-w-4xl mx-auto text-center">
                ...
                <Link to="/create-lot"
                      className="bg-green-600 text-white px-8 py-3 rounded-xl font-bold hover:bg-green-700 transition shadow-lg inline-block">
                    + Выставить новый лот
                </Link>
                <div className="mt-8 space-y-4 text-left max-w-2xl mx-auto">
                    ...
                    {myLots.map(l => (
                        <div key={l.id}
                             className="p-4 border border-gray-200 rounded-xl flex justify-between items-center bg-white shadow-sm hover:shadow transition">
        ...
        <div className="p-8 max-w-6xl mx-auto mb-20 text-left">
            ...
            <div className="flex flex-wrap gap-2 mb-8 bg-gray-100 p-1 rounded-xl w-fit">
                <button onClick={() => setActiveTab('kyc')}
                        className={`flex items-center gap-2 px-4 py-2 rounded-lg font-bold transition ${activeTab === 'kyc' ? 'bg-white shadow text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}>
                    <FileCheck size={18}/>Заявки
                </button>
                <button onClick={() => setActiveTab('users')}
                        className={`flex items-center gap-2 px-4 py-2 rounded-lg font-bold transition ${activeTab === 'users' ? 'bg-white shadow text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}>
                    <Users size={18}/>Юзеры
                </button>
                <button onClick={() => setActiveTab('lots')}
                        className={`flex items-center gap-2 px-4 py-2 rounded-lg font-bold transition ${activeTab === 'lots' ? 'bg-white shadow text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}>
                    <Gavel size={18}/>Лоты
                </button>
                <button onClick={() => setActiveTab('categories')}
                        className={`flex items-center gap-2 px-4 py-2 rounded-lg font-bold transition ${activeTab === 'categories' ? 'bg-white shadow text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}>
                    <FolderTree size={18}/>Категории
                </button>
                <button onClick={() => setActiveTab('admins')}
                        className={`flex items-center gap-2 px-4 py-2 rounded-lg font-bold transition ${activeTab === 'admins' ? 'bg-white shadow text-red-600' : 'text-gray-500 hover:text-gray-700'}`}>
                    <ShieldPlus size={18}/>Новый админ
                </button>
            </div>
            ...
            {activeTab === 'kyc' && (
                <div className="grid gap-4">
                    {applications.length === 0 &&
                        <p className="text-gray-400 italic">Новых заявок на верификацию нет</p>}
                    {applications.map(app => (
                        <div key={app.id}
                            ...
                            <div>
                                ...
                                <a href={`http://localhost:8080/${app.documentPath}`} target="_blank" rel="noreferrer"
                                   className="text-blue-600 text-xs font-bold underline">Просмотреть документ</a>
                            </div>
                            <button onClick={() => handleApprove(app.id)}
                                    className="bg-green-100 text-green-700 px-4 py-2 rounded-lg font-bold hover:bg-green-200 transition">Одобрить
                            </button>
                        </div>
                    ))}
                </div>
                        ...
                        {users.map(u => (
                            <tr key={u.id} className="border-b last:border-0 hover:bg-gray-50">
                                ...
                                <td className="p-4"><span
                                    className="text-xs font-bold px-2 py-1 bg-gray-100 rounded text-gray-600">{u.role}</span>
                                </td>
                                <td className="p-4">
                                    <span
                                        className={`text-xs font-bold ${u.status === 'ACTIVE' ? 'text-green-600' : 'text-red-600'}`}>{u.status}</span>
                                </td>
                                <td className="p-4">
                                    <button
                    ...
                    <table className="w-full text-left">
                        ...
                        {lots.map(l => (
                            <tr key={l.id} className="border-b last:border-0 hover:bg-gray-50">
                                ...
                                <td className="p-4"><span
                                    className="text-xs px-2 py-1 bg-gray-100 rounded text-gray-500 font-bold">{l.status}</span>
                                </td>
                                <td className="p-4">
                                    <button onClick={() => deleteLot(l.id)}
                                            className="text-red-500 hover:bg-red-50 p-2 rounded-lg transition"><Trash2
                                        size={20}/></button>
                                </td>
                            </tr>
                        ))}
            ...
            {activeTab === 'admins' && (
                <div className="max-w-md bg-white p-8 rounded-2xl shadow-sm border border-red-100 text-left">
                    ...
                    <form onSubmit={createAdmin} className="space-y-4">
                        ...
                        <button type="submit"
                                className="w-full bg-red-600 text-white py-3 rounded-xl font-bold hover:bg-red-700 transition shadow-md mt-2">Создать
                            аккаунт
                        </button>
                    </form>
                </div>
