                            ...
                            {user ? (
                                <div className="flex items-center gap-4">
                                    {user?.role === 'ROLE_ADMIN' && (
                                        <Link to="/admin"
                                              className="text-sm font-bold text-red-600 border border-red-200 px-3 py-1 rounded-lg hover:bg-red-50">
                                            Админка
                                        </Link>
                                    )}
                                    ...
                                    {user?.role === 'ROLE_SELLER' && (
                                        <>
                                            <Link to="/admin"
                                                  className="text-sm font-bold text-green-600 border border-green-200 px-3 py-1 rounded-lg hover:bg-green-50">
                                                Мои лоты
                                            </Link>
                                        </>
                                    )}
                                    {user?.role === 'ROLE_BUYER' && (
                                        <Link to="/apply-seller"
                                              className="text-sm font-bold text-blue-600 border border-blue-200 px-3 py-1 rounded-lg hover:bg-blue-50">
                                            Стать продавцом
                                        </Link>
                                    )}
                                    {user?.role === 'ROLE_BUYER' && (
